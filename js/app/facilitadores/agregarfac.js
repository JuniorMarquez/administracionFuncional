...
app.controller('AgregarfacCtrl', ['$scope', '$state','$http', '$filter', '$modal', 'MyService', 'filterFilter', 'toaster','$timeout', 'FileUploader', function($scope,  $state ,$http, $filter,$modal, MyService, filterFilter, toaster,$timeout, FileUploader) {
 ...
 $scope.add = function () {
  ...
  $scope.guardarFacilitador = function(item){
    ...
    facilitadorAct.idUsuario=MyService.data.idUsuario;
      ...
      $http.post('http://52.39.15.75:1346/facilitador/', facilitadorAct).success(function(data){
