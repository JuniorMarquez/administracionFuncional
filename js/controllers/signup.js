...
app.controller('SignupFormController', ['$scope', '$filter','$http', '$state','MyService', 'toaster','$modal', function($scope, $filter,$http, $state,MyService,toaster,$modal) {
  ...
  $scope.pop = function(){
    ...
    $scope.createEstablecimiento = function(user){
       $http.post('http://52.39.15.75:1346/establecimiento/', {administrador: $scope.user.nombre, emailAdministrador: $scope.user.email});
    };

    $scope.entrar = function(user) {
      $http.get('http://52.39.15.75:1346/userbcsc/?email=' +$scope.user.email).success(function(respuesta){
...
MyService.letra="e";
  ...
  // item.tipo=MyService.data.tipo;
    ...
    $scope.okNuevoMiembro = function(user) {
      $http.post('http://52.39.15.75:1346/miembro/', {
